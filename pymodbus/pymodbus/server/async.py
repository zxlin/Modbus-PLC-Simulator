...
class ModbusTcpProtocol(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        if _logger.isEnabledFor(logging.DEBUG):
            _logger.debug("Client [%s] requested" %self.transport.getHost() + " ".join([hex(ord(x)) for x in data]))
    ...
    def _send(self, message):
        ...
        if message.should_respond:
            ...
            if _logger.isEnabledFor(logging.DEBUG):
                _logger.debug('Server responded to [%s]: %s' % (self.transport.getHost(), b2a_hex(pdu)))
